...
export const HeadComplaintsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-slate-800">Department Complaints</h1>
      {complaints.length === 0 ? (
        <div className="card"><EmptyState title="No complaints in your department yet" /></div>
      ) : (
        <div className="card overflow-x-auto p-0">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-100">
                <th className="py-3 px-4">ID</th><th className="py-3 px-4">Title</th><th className="py-3 px-4">Priority</th><th className="py-3 px-4">Status</th>
              </tr>
            </thead>
            <tbody>
              {complaints.map((c) => (
